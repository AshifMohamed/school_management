...
public class StudentManagementDBUtils {
    ...
    public static void main(String[] args) throws SQLException {     
        System.out.println(getPaidAmount("PT_1"));
    }

    DateFormat oDateFormat = new SimpleDateFormat("yyyy-MM-dd");

    public void saveStudent(String firstName, String lastName, String middleName, String dateOfBirth, String address, String nationality, String gender, String eMail, String contactNumber ,String applicantId,int currentGrade) {
        try (Connection dbConnection = DBConn.myConn()) {
            dbConnection.createStatement().executeUpdate("insert into student values('" + getStudentId() + "','" + oDateFormat.format(new Date()) + "','" + firstName + "','" + middleName + "','" + lastName + "','" + dateOfBirth + "','" + address + "','" + gender + "','" + nationality + "','" + contactNumber + "','" + eMail + "','active','" + applicantId + "',,'" + currentGrade + "')");
     ...
     public boolean saveApplicant(String applicantName, String date, String contactNo, String grade,int marks,String hallName,String time ,String status) {
         boolean saved=false;
        try (Connection dbConnection = DBConn.myConn()) {
